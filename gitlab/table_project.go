...
func tableProject() *plugin.Table {
	return &plugin.Table{
		...
		Get: &plugin.GetConfig{
			//KeyColumns: plugin.AnyColumn([]string{"id", "full_path"}),
		...
		Columns: []*plugin.Column{
			...
			{Name: "public", Type: proto.ColumnType_BOOL, Description: "Is the project public"},
			...
			{Name: "creator_id", Type: proto.ColumnType_INT, Description: "User ID of the projects creator."},
			...
			{Name: "owner_username", Type: proto.ColumnType_STRING, Description: "The projects owner username. (null if owned by a group)", Transform: transform.FromField("Owner.Username")},
		},
	}
}
...
func getProject(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	//fullPath := quals["full_path"].GetStringValue()

	opt := &api.GetProjectOptions{}

	//project, _, err := conn.Projects.GetProject(getProjectIdentifier(id, fullPath), opt)
...
//func getProjectIdentifier(id int, path string) interface{} {
//	if path != "" {
//		return path
//	}
//	return id
